...
int main(int argc, const char** argv) {
  if (argc != 5) {
    std::cout << "usage: ./yolov3 <weights_file> <coco_names> <input_image> <output_image>\n";
    return -1;
  }
  ...
  for (size_t box = 0; box < numBoxes; box++)
  {
    ...
    BoundingBox bbox = {
      .pos = pos,
      .color = red,
      .objectClass = model.ClassNames()[classIndex],
      .objectProb = objectProb
    };
    bboxes.push_back(bbox);
  }

  NonMaxSuppression(bboxes, nmsThresh);
